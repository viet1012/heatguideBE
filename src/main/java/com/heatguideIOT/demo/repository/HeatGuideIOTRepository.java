    ...
    @Query(value = """
     WITH RankedLots AS (
             ...
             , RankedData AS (
                 ...
                 GROUP BY
                     l.lot, d.POREQNO, d.FERTH, d.ITEMCHECK
             )
             SELECT
                 lot, FERTH, ITEMCHECK, STARTTIME, FINISHTIME
             FROM RankedData
    ...
    @Query(value = """
        WITH RankedLots AS (
                ...
                , RankedData AS (
                    ...
                    LEFT JOIN F2_HeatGuide_Daily d2
                        ON d.POREQNO = d2.POREQNO
                        AND d2.ITEMCHECK = 'Waiting'  -- Chỉ lấy các mục đã có 'Waiting'
                    LEFT JOIN HeatFinishGuide hfg
                        ON hfg.PO = l.POREQNO
                    WHERE
                        d.FERTH IN ('Mold Bush', 'Main Bush', 'Sub Post', 'Sub Bush', 'Dowel Pins')
      ...
      WITH RankedLots AS (
             ...
             , RankedData AS (
                 ...
                 WHERE\s
                     d.FERTH IN ('Mold Post', 'Main Post')
                     AND d.STARTTIME >= DATEADD(DAY, -7, GETDATE())
                     AND d2.POREQNO IS NULL \s
                     AND hfg.PO IS NULL     \s
                     AND l.lot_rank = 1  -- Chỉ lấy lot nhỏ nhất cho mỗi POREQNO
                 GROUP BY\s
                     l.lot, d.POREQNO, d.FERTH, d.ITEMCHECK
             )
